...
db.serialize(() => {
  ...
  db.run(`
    CREATE TABLE IF NOT EXISTS stream_containers (
        ...
        preview_file_video TEXT,    -- File path untuk preview video
        preview_file_audio TEXT,    -- File path untuk preview audio
        stream_file_video TEXT,           -- File path untuk streaming video
        stream_file_audio TEXT,           -- File path untuk streaming audio
        ...
        audio_enabled INTEGER DEFAULT 0, -- 0 atau 1 sebagai boolean
...
const addStreamContainer = (data, callback) => {
  db.run(
    ...
    [
      ...
      data.preview_file_audio,
      data.stream_file_video,
      data.stream_file_audio,
      data.stream_key,
      data.stream_url,
      data.bitrate,
      data.resolution,
...
const updateStreamContainer = (id, updates, callback) => {
  ...
  if (updates.stream_file_video) {
    setClauses.push("stream_file_video = ?");
    params.push(updates.stream_file_vieo);
  }
  if (updates.stream_file_audio) {
    setClauses.push("stream_file_audio = ?");
    params.push(updates.stream_file_audio);
  }
...
const getActiveStreamContainers = (callback) => {
  db.all(
    "SELECT * FROM stream_containers WHERE is_streaming = 1 ORDER BY id ASC",
    [],
    callback
  );
};
...
const getStreamContainerByStreamKey = (streamKey, callback) => {
  db.get(
    "SELECT * FROM stream_containers WHERE stream_key = ?",
    [streamKey],
    callback
  );
};
...
const getHistoryStreamContainers = (callback) => {
  db.all(
    "SELECT * FROM stream_containers WHERE is_streaming = 0",
    [],
    callback
  );
};
